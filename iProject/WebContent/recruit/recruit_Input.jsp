...
$(document).ready(function(){
	<c:if test="${recruitVO.inputType == 'update'}">
		$("#BtnSave").hide(); 
		$("#BtnUpdate").show();
	</c:if>
	<c:if test="${recruitVO.inputType == 'insert'}">
		$("#BtnSave").show();
		$("#BtnUpdate").hide(); 
	</c:if>
...
function BtnList(){
	$("#form").attr("action","Recruit_List").submit();
}
function BtnSave(){
	$("#form").attr("action", "Recruit_InputSubmit").submit();
}
function BtnUpdate(){
	$("#form").attr("action", "Recruit_InputSubmit").submit();
}
        ...
        <div class="contents_wrap">
            ...
            <div class="contents mt120">
				<form name="form" id="form" action="" method="post" enctype="multipart/form-data" >
					<input type="hidden" name="inputType" value="${recruitVO.inputType}" />
					...
					<input type="hidden" name="sseq" id="sseq" value="${recruitVO.sseq}" /> 
				 	<table width="764" cellpadding="0" cellspacing="0" class="bbs_view" >
						...
						<tbody>
							<tr>
	                    	<th>제목</th>
		                    	<td><input type="text" id="subject" name="subject" title="제목을 입력하세요" class="text" value="${recruitVO.subject}" style="width:97%;"></td>
		                	</tr>
							<tr>
								<th>작성자</th>
								<td>Inection</td>
		                	...
		                	<tr>
		                    	<th>내용</th>
		                    	<td><textarea name="content" id="content" cols="30" rows="10" style="width:97%; height:200px; margin:8px 0 8px 0;" class="area"><c:if test="${recruitVO.inputType == 'update'}">${recruitVO.content}</c:if></textarea></td>
		                    </tr>
						</tbody>
					</table>
					<p class="btn_center">
